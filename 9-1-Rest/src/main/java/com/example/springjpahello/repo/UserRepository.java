...
import com.example.springjpahello.constants.Constants;
...
public interface UserRepository extends JpaRepository<User, Long>, Constants {
...
//    @Query(value = "{'firstname': {$regex : ?0, $options: 'i'},'lastname':{$regex : ?1, $options: 'i'}}")
  //  List<User> findByFirstnameRegexOrLastnameRegex(String regexString,String lastnameRegexString);
    ...
    @Query(value = userSelectNative, nativeQuery = true)
    ...
    @Modifying
    @Query("delete from User u where u.emailAddress=:email")
    public void deleteB(@Param("email") String emailAddress);

    List<User> findAllOrderedDescending();
    List<User> findFistAndLast(String firstname,String lastname);
